...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, need_feature=False):
        ...
        if need_feature:
            x1 = self.decoder(x, features)
            logits = self.segmentation_head(x1)
            features[0] = nn.Dropout2d(0.5)(features[0])
            features[1] = nn.Dropout2d(0.5)(features[1])
            features[2] = nn.Dropout2d(0.5)(features[2])

            x_fp = self.decoder(x, features)
            logits_fp = self.segmentation_head(x_fp)
